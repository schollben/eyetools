...
def load_eye_quality(analyzable_output_dir: Path, threshold: str) -> dict:
    ...
    return {
        ...
        "LEQ": df.loc[(df.trajectory == "left_eye_data_quality") & (df.component == f"{threshold}_threshold"), "value"].values.astype(bool),
        "REQ": df.loc[(df.trajectory == "right_eye_data_quality") & (df.component == f"{threshold}_threshold"), "value"].values.astype(bool),
    }
